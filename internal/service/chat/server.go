...
func normalizePath(path string) string {
	...
	if staticIndex < 0 {
		...
		// 安全修复：返回默认头像而非原路径，避免用户注入外部追踪 URL
		...
		case data := <-s.Transmit:
			{
				...
				if chatMessageReq.Type == message_type_enum.Text {
					...
					if message.ReceiveId[0] == 'U' { // 发送给User
						...
						s.mutex.RLock()
						receiveClient, recvOk := s.Clients[message.ReceiveId]
						sendClient, sendOk := s.Clients[message.SendId]
						s.mutex.RUnlock()
						if recvOk {
							_ = receiveClient.EnqueueDelivery(messageBack)
						}
						if sendOk {
							_ = sendClient.EnqueueDelivery(messageBack)
						}
						...
						for _, c := range targets {
							_ = c.EnqueueDelivery(messageBack)
						}
