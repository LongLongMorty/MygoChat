                      ...
                      <div class="left-message-file-container">
                        ...
                        <div class="left-message-file-download">
                          <el-button
                            ...
                            "
                            size="small"
                            @click="downloadFile(messageItem.url)"
                          >
                            下载
                          </el-button>
                        </div>
  ...
  setup() {
    ...
    const sendFileMessage = async (fileUrl, serverName, originalName) => {
      const chatFileMessageRequest = {
        ...
        file_name: originalName || data.fileList[0].name, // 原始名用于展示
    ...
    const handleUploadSuccess = (response) => {
      // P1 修复：使用后端返回的服务端文件名和鉴权下载 URL
      if (response.code === 0 && response.data && response.data.url) {
        ElMessage.success("文件上传成功");
        // 传 URL、server_name（用于下载）、原始名（用于展示）
        sendFileMessage(
          store.state.backendUrl + response.data.url,
          response.data.name,
    ...
    const handleAvatarUploadSuccess = (response) => {
      ...
      if (response.code === 0 && response.data && response.data.name) {
        ...
        data.uploadedAvatarName = response.data.name;
        // 如果有待处理的群信息更新，使用服务端文件名继续
        if (data.pendingGroupUpdate) {
          data.pendingGroupUpdate = false;
          data.updateGroupInfo.avatar = "/static/avatars/" + response.data.name;
          doUpdateGroupInfo();
        }
      } else {
    ...
    const downloadFile = async (fileUrl) => {
      try {
        // P1 修复：从 url 中提取 server name 用于鉴权下载
        // fileUrl 格式：https://host/file/download?name=file_1234567890.png
        let serverName = "";
        try {
          const urlObj = new URL(fileUrl);
          serverName = urlObj.searchParams.get("name") || "";
        } catch (e) {
          // 如果不是完整 URL，尝试直接解析
          serverName = fileUrl;
        }
        const rsp = await axios.get(
          store.state.backendUrl + "/file/download?name=" + serverName,
          {
            responseType: "blob",
          }
        );
    ...
    const handleUpdateGroupInfo = async () => {
      try {
        ...
        // P2 修复：有头像时先上传，等 handleAvatarUploadSuccess 回调完成后继续
        if (data.avatarList.length > 0) {
          data.pendingGroupUpdate = true;
          data.uploadAvatarRef.submit();
          return; // 等待上传完成，handleAvatarUploadSuccess 会继续
        }
        await doUpdateGroupInfo();
      } catch (error) {
        console.error(error);
    ...
    // P1 修复：el-upload 需要显式传 Authorization header
